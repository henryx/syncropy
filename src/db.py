...
class DBManager(object):
    ...
    def _create_schema(self, connection, system):
        pragmas = [
        ...
        if system:
            tables = ["CREATE TABLE status (grace VARCHAR(5), actual INTEGER, last_run TIMESTAMP)"]
            data = [
    ...
    def open(self, dbname, system=False):
        connection = sqlite.connect(dbname)

        if not self._check_schema(connection):
            self._create_schema(connection, system)
